...
func (d *Downloader) Synchronise(id string, head common.Hash, number *big.Int, mode SyncMode) error {
...
func (d *Downloader) synchronise(id string, hash common.Hash, number *big.Int, mode SyncMode) error {
	// end change by Shara
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, number *big.Int) (err error) {
...
func (d *Downloader) processHeaders(origin uint64, pivot uint64, number *big.Int) error {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				if d.mode != LightSync {
					...
					if !gotHeaders && number.Cmp(head.Number()) > 0 {
