...
func TestAccountListEmpty(t *testing.T) {
	gdacc := runGeth(t, "account", "list")
	gdacc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gdacc := runGeth(t, "account", "list", "--datadir", datadir)
	defer gdacc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gdacc := runGeth(t, "account", "new", "--lightkdf")
	defer gdacc.ExpectExit()
	gdacc.Expect(`
...
`)
	gdacc.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gdacc := runGeth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gdacc := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gdacc.ExpectExit()
	gdacc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gdacc.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gdacc.StderrText(), m) {
...
// https://github.com/daccproject/go-dacc/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gdacc.SetTemplateFunc("keypath", func(file string) string {
