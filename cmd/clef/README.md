Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for gdacc's account management.
This allows DApps not to depend on gdacc's account management. When a DApp wants to sign data it can send the data to
...
The general flow for signing a transaction using e.g. gdacc is as follows:
![image](sign_flow.png)

In this case, `gdacc` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Gdacc should be started in `gdacc --external_signer localhost:8550`.
    - Currently, the Gdacc APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Gdacc api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus gdacc/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Gdacc. The messages are
...
The signer will generate a new private key, encrypts it according to [web3 keystore spec](https://github.com/daccproject/wiki/wiki/Web3-Secret-Storage-Definition) and stores it in the keystore directory.
...
#### Arguments
  - account [object]: key in [web3 keystore format](https://github.com/daccproject/wiki/wiki/Web3-Secret-Storage-Definition) (retrieved with account_export) 
...
#### Result
  - exported key, see [web3 keystore format](https://github.com/daccproject/wiki/wiki/Web3-Secret-Storage-Definition) for
  more information
