...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/daccproject/go-dacc

    go install github.com/daccproject/go-dacc/cmd/swarm
...
You must have your working copy under `$GOPATH/src/github.com/daccproject/go-dacc`.

Most likely you will be working from your fork of `go-dacc`, let's say from `github.com/nirname/go-dacc`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-dacc.git $GOPATH/src/github.com/daccproject/go-dacc
```
...
```
stateth --rm --grafana-dashboards-folder $GOPATH/src/github.com/daccproject/go-dacc/swarm/grafana_dashboards --influxdb-database metrics
```
 ...
 * [Code review guidelines](https://github.com/daccproject/go-dacc/wiki/Code-Review-Guidelines).
