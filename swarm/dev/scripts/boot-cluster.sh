...
#
# The cluster contains a bootnode, a gdacc node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gdacc and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gdacc node with --datadir pointing at <base-dir>/gdacc
# and a single, unlocked account with password "gdacc"
start_geth_node() {
  local dir="${base_dir}/gdacc"
  mkdir -p "${dir}"

  local password="gdacc"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating gdacc account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gdacc account address"
  fi
  ...
  start_node "gdacc" "${GETH_IP}" "$(which gdacc)" ${args[@]}
}
...
create_account() {
  ...
  gdacc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
