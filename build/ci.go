...
import (
	...
	"github.com/daccproject/go-dacc/internal/build"
	"github.com/daccproject/go-dacc/params"
	sv "github.com/daccproject/go-dacc/swarm/version"
)

var (
	// Files that end up in the gdacc*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gdacc"),
	}

	// Files that end up in the gdacc-alltools*.zip archive.
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 9 {
			...
			log.Println("go-dacc requires at least Go version 1.9 and cannot")
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gdacc-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debMetadata struct {
	...
	// go-dacc version being built. Note that this
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gdacc.exe" {
			gethTool = file
		} else {
	...
	installer, _ := filepath.Abs("gdacc-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gdacc.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/daccproject/go-dacc/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gdacc.aar", filepath.Join(GOBIN, "gdacc.aar"))
		return
	}
	...
	archive := "gdacc-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gdacc.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "--tags", "ios", "-v", "github.com/daccproject/go-dacc/mobile")
	...
	archive := "gdacc-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
