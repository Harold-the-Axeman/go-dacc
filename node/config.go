...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gdacc is "gdacc". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gdacc", keep that.
	if name == "gdacc" || name == "gdacc-testnet" {
		name = "Gdacc"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gdacc" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gdacc 1.4 are used if they exist.
	if c.name() == "gdacc" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "gdacc" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-dacc-keystore")
		ephemeral = keydir
	}
