...
const (
	...
	DefaultHTTPPort = 8787        // Default TCP port for the HTTP RPC server
	DefaultWSHost   = "localhost" // Default host interface for the websocket RPC server
	DefaultWSPort   = 8788        // Default TCP port for the websocket RPC server
)
...
func DefaultDataDir() string {
	...
	if home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "DACC")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Roaming", "DACC")
		} else {
			return filepath.Join(home, ".DACC")
		}
	}
